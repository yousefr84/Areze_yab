...
def openEnded(report_id, national_id, questionnaire_id):
    try:
        ...
        response = ''
        prompt = f'''
        تصور کن یک مشاور مالی و مدیریت هستی و قصد داری بر اساس مدل دوپونت و با توجه به اطلاعات صورت سود و زیان و ترازنامه، یک تحلیل مالی و مدیریتی به مدیر عامل یک شرکت که در زمینه {questionnaire.company.company_domain}فعالیت میکند ارائه بدی طوری که براساس گزارش مدیر بتواند تصمیمات صحیحی برای استراتژی شرکت بگیرد   . اطلاعات مالی به واحد ریال و به شرح زیر است:
        {'\n'.join(messages)}
...
پارگراف سوم: (تحلیل عملکرد مالی شرکت بر اساس مدل دوپونت و با توجه به زمینه فعالیت شرکت )
گزارش هر اندازه هم که طولانی شد اما به صورت کامل و بدون وقفه تحلیل را ارائه بده
    ...
    '''
        ...
        response += 'end first'
        prompt = f'''
       تصور کن یک مشاور ارشد مدیریت مالی و مدیریت هستی و قصد داری بر اساس مدل دوپونت و با توجه به اطلاعات صورت سود و زیان و ترازنامه، یک تحلیل مالی و مدیریتی برای مدیرعامل شرکتی که در زمینه {questionnaire.company.company_domain}فعالیت میکند طوری که او بتونه برنامه استراتژی کسب وکار خودشو با توجه به زمینه فعالیتش اصلاح کنه و نقاط قوت و ضعف شرکت رو بدونه ارائه بدی. اطلاعات مالی به واحد ریال و به شرح زیر است:
        {'\n'.join(messages)}
        با توجه به اطلاعات مالی، همچنین برای تاثیرگذاری بهتر تحلیل دوپونت در تصمیم گیری ها، راهکارهایی برای بهبود نسبت هایی که محاسبه کردی ارائه بده . برای هر شاخص ، 3 تا 5 پیشنهاد منحصر به فرد برای بهبود عملکرد آن شاخص بده به گونه ای که با این‌گزارش که برای مدیرعامل شرکت که آشنایی عمومی با مفاهیم مالی داره توسط تو که مشاور مالی و اجرایی مدیر هستی آماده شده بتونه برای استراتژی آینده شرکت برنامه ریزی کلان کنه بدیهی هست که این گزارش باید به زبان ساده اما از دیدگاه تخصصی یک‌مشاور نوشته بشه و کل گزارش و مثال ها با توجه به زمینه فعالیت شرکت نوشته شود
...
به طور مثال: (توضیح مثال عملیاتی)
پیشنهاد پنجم: (توضیح پیشنهاد بهبود)
تاکید میکنم گزارش هر اندازه هم که طولانی شد اما به صورت کامل و بدون وقفه راهکارها و مثال ها را ارائه بده
        '''
        response += openAI(prompt, rule)
        print(prompt)

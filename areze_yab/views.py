...
class HistoryViewSet(ViewSet):
    ...
    def Mentor(self, request, nationalID):
        try:
            ...
            return {'error': 'user id missing'}  # داده‌ها به صورت دیکشنری بازگردانده می‌شود، نه Response

        return UserSerializer(user).data  # بازگرداندن داده‌ها به صورت دیکشنری

    @action(detail=False, methods=['get'])
    def All(self, request):
        objects = []
        is_company = request.query_params.get('is_company')
        nationalID = request.query_params.get('username')
        if not nationalID:
        ...
        for company in companies:
            ...
            objects.append({
                "sales_and_marketing_s": SalesAndMarketingSSerializer(
                    SalesAndMarketingS.objects.filter(company_id=companyID), many=True).data,
                "sales_and_marketing_m": SalesAndMarketingMSerializer(
                    SalesAndMarketingM.objects.filter(company_id=companyID), many=True).data,
                "sales_and_marketing_l": SalesAndMarketingLSerializer(
                    SalesAndMarketingL.objects.filter(company_id=companyID), many=True).data,
                "branding_s": BrandingSSerializer(BrandingS.objects.filter(company_id=companyID), many=True).data,
                "branding_m": BrandingMSerializer(BrandingM.objects.filter(company_id=companyID), many=True).data,
                "branding_l": BrandingLSerializer(BrandingL.objects.filter(company_id=companyID), many=True).data,
                "human_resources_s": HumanResourcesSSerializer(HumanResourcesS.objects.filter(company_id=companyID),
                                                               many=True).data,
                "human_resources_l": HumanResourcesLSerializer(HumanResourcesL.objects.filter(company_id=companyID),
                                                               many=True).data,
                "human_resources_m": HumanResourcesMSerializer(HumanResourcesM.objects.filter(company_id=companyID),
                                                               many=True).data,
            })
        print(f"type:{type(is_company)}")
        print(f"valiu:{is_company}")
        if is_company == "false":
            mentor = self.Mentor(request, nationalID)
...
class BaseAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response(data="Company does not exist", status=status.HTTP_400_BAD_REQUEST)
        ...
        if size == 'small':
            ...
            return Response(data="some error with size", status=status.HTTP_400_BAD_REQUEST)
        results = {}
        ...
        for subdomain, fields in subdomains.items():
            if not fields:
                continue  # اگر فیلدها وجود نداشته باشند، آن زیر دامنه را نادیده بگیر
        ...
        prompt = f'''تصور کن به عنوان یک کارشناس در حوزه {self.domain} قصد داری {self.domain} یک شرکت با مقیاس {company.size} را عارضه یابی کنی. برای این کار پرسشنامه عارضه یابی {self.domain} در اختیار تو است. این پرسشنامه از حوزه های مختلف تشکیل شده و هر حوزه سوالات مربوط به خود که نشان دهنده ی شاخص های عملکردی حوزه {self.domain} است را   شامل می‌شود. هر حوزه را با توجه به پاسخ های داده شده ی مربوط به آن در 120 کلمه و 2 پاراگراف تحلیل کن. این تحلیل باید شامل گزارشی از وضعیت فعلی حوزه {self.domain}، نقاط قوت و ضعف و فرصت‌های بهبود باشد.  
        همچنین برای هر حوزه، 3 تا 5 پیشنهاد منحصر به فرد برای بهبود عملکرد آن حوزه به گونه ای که برای صاحب کسب و کار با آشنایی اولیه در حوزه {self.domain} و به صورت ساده ارائه بده و برای هر پیشنهاد به صورت 
        ...
        پرسشنامه عارضه یابی {self.domain} به شرح زیر است:
        {subdomain_results}
        با توجه به پاسخ‌ها، در ابتدای گزارش عارضه یابی عملکرد شرکت را در حوزه {self.domain} که در سطح {company.size} قرار دارد را با در نظر گرفتن همه ی حوزه‌ها تحلیل کن و توضیحات را در 300 کلمه و 3 پاراگراف ارائه بده. در نظر داشته باش که این شرکت در زمینه {company.company_domain} فعالیت میکند. در تحلیل ها و راهکارهای پیشنهادی خود، زمینه {company.company_domain} را در نظر داشته باش.
        متنی از خودت اضافه نکن و فقط گزارش را ارائه بده.   
        '''
        rule = f"تصور کن به عنوان یک کارشناس در حوزه {self.domain} قصد داری {self.domain} یک شرکت با مقیاس {company.size} را عارضه یابی کنی"
...
class SalesAndMarketingAPIView(BaseAPIView):
    ...
    small_finall = "employee_professional_ethics"
    medium_final = "using_discounts_and_special_offers_for_sales_promotion"
    large_finall = "advertising_adjustment_based_on_customer_needs"
...
class HumanResourceAPIView(BaseAPIView):
    ...
    small_subdomains = {
        "تعداد نیروی انسانی": ["staffing_sufficiency",
                               "recruitment_planning"],
        "ماندگاری نیروی انسانی": ["employee_turnover_rate",
                                  "employee_turnover_reasons_attention"],
        "سیستم و ارزیابی عملکرد کارکنان": ["employee_performance_evaluation",
                                           "employee_productivity_tool",
                                           "performance_evaluation_impact"],
        "سیستم و آموزش کارکنان": ["employee_training_programs",
                                  "training_impact_on_performance"],
        "همکاری و کار گروهی": ["employee_collaboration_effectiveness",
                               "teamwork_culture_exists"],
        "توجه به ارزش های اخلاقی": ["ethical_standards_attention",
                                    "employee_professional_ethics"]
    }

    medium_subdomains = {
        "تعداد نیروی انسانی": ["staffing_vs_workload",
                               "recruitment_plan"],
        "ماندگاری نیروی انسانی": ["employee_retention_level",
                                  "employee_exit_reasons_analysis",
                                  "key_employee_retention_plan"],
        "بهره‌وری پرسنل": ["employee_performance_measurability",
                           "productivity_evaluator",
                           "employee_exit_alignment_with_goals"],
        "سیستم و ارزیابی عملکرد کارکنان": ["structured_performance_evaluation",
                                           "performance_evaluation_salary_promotion_impact",
                                           "performance_evaluation_impact_on_salary_promotion"],
        "سیستم و آموزش کارکنان": ["continuous_employee_training",
                                  "training_effectiveness_evaluation",
                                  "training_budget_allocation"],
        "همکاری و کار گروهی": ["interdepartmental_collaboration",
                               "collaboration_enhancement_programs",
                               "teamwork_culture"],
        "توجه به ارزش های اخلاقی": ["ethical_behavior_of_employees",
                                    "managers_as_role_models_for_ethics",
                                    "professional_ethics_code"]
    }

    large_subdomains = {
        "تعداد نیروی انسانی": ["current_staff_meets_workload",
                               "organizational_structure_updated_for_growth"],
        "ماندگاری نیروی انسانی": ["employee_turnover_rate_last_year",
                                  "employee_satisfaction_loyalty_program"],
        "بهره‌وری پرسنل": ["key_department_productivity_evaluation",
                           "resources_for_productivity_increase"],
        "سیستم و ارزیابی عملکرد کارکنان": ["formal_performance_evaluation_system",
                                           "feedback_leads_to_performance_improvement"],
        "سیستم و آموزش کارکنان": ["structured_training_program_exists",
                                  "training_leads_to_performance_improvement"],
        "همکاری و کار گروهی": ["team_collaboration_level",
                               "team_conflict_management"],
        "توجه به ارزش های اخلاقی": ["ethical_values_in_behavior",
                                    "ethical_values_in_recruitment"]
    }
    small_finall = "employee_professional_ethics"
    medium_final = "professional_ethics_code"
    large_finall = "ethical_values_in_recruitment"
...
class BrandingAPIView(BaseAPIView):
    ...
    domain = 'برندینگ'
    small_finall = "current_brand_innovation"
    medium_final = "regular_brand_performance_evaluation"
    large_finall = "use_loyal_customers_for_brand_awareness"
    small_subdomains = {
        "Brand identity": ["brand_identity_clarity",
                           "staff_brand_awareness"],
        "Brand awareness": ["customer_brand_awareness",
                            "social_media_presence"],
        "Customer experience": ["customer_brand_experience",
                                "customer_feedback_usage"],
        "Brand loyalty": ["customer_loyalty_programs",
                          "repeat_purchase_frequency",
                          "willingness_to_pay_more"],
        "Brand evaluation": ["brand_performance_evaluation",
                             "customer_data_for_performance"],
        "Brand strategy": ["brand_strategy_documentation",
                           "continuous_brand_investment"],
        "Innovation in branding": ["current_brand_innovation"]
    }
    small_numeric_fields = {
        "Brand identity": ["brand_identity_clarityNum",
                           "staff_brand_awarenessNum"],
        "Brand awareness": ["customer_brand_awarenessNum",
                            "social_media_presenceNum"],
        "Customer experience": ["customer_brand_experienceNum",
                                "customer_feedback_usageNum"],
        "Brand loyalty": ["customer_loyalty_programsNum",
                          "repeat_purchase_frequencyNum",
                          "willingness_to_pay_moreNum"],
        "Brand evaluation": ["brand_performance_evaluationNum",
                             "customer_data_for_performanceNum"],
        "Brand strategy": ["brand_strategy_documentationNum",
                           "continuous_brand_investmentNum"],
        "Innovation in branding": ["current_brand_innovationNum"]
    }

    medium_subdomains = {
        "هویت برند": ["clear_brand_identity",
                      "brand_identity_alignment_with_strategy",
                      "brand_identity_for_cultural_clarity"],
        "آگاهی از برند": ["brand_recognition_in_target_market",
                          "digital_advertising_for_brand_awareness",
                          "advertising_campaigns_for_brand_awareness",
                          "brand_association_with_quality"],
        "تجربه مشتری": ["market_research_for_customer_experience",
                        "customer_feedback_for_brand_experience",
                        "processes_for_measuring_customer_experience",
                        "customer_feedback_for_product_service_improvement"],
        "وفاداری برند": ["special_programs_for_customer_loyalty",
                         "loyal_customers_for_brand_promotion",
                         "customer_loyalty_rewards"],
        "ارزیابی برند": ["regular_brand_performance_evaluation"]
    }

    large_subdomains = {
        "هویت برند": ["brand_identity_alignment_with_culture",
                      "employee_involvement_in_brand_identity",
                      "brand_identity_implementation_at_touchpoints"],
        "آگاهی از برند": ["use_of_various_ad_campaigns_for_brand_awareness",
                          "use_of_multiple_channels_for_brand_promotion",
                          "use_of_data_and_analytics_in_ad_campaign_evaluation",
                          "regular_brand_awareness_evaluation"],
        "تجربه مشتری": ["market_research_for_brand_experience",
                        "using_customer_data_for_brand_personalization",
                        "regular_customer_experience_evaluation",
                        "using_customer_feedback_for_brand_experience_improvement"],
        "وفاداری برند": ["has_loyalty_program_for_customers",
                         "use_loyal_customers_for_brand_awareness"]
    }
    small_questions = {
        "Brand identity": [
        ...
        "Brand awareness": ["3. آیا مشتریان شما برند شما را به\u200Cخوبی می\u200Cشناسند و به خاطر می\u200Cآورند؟",
                            "4.آیا شما در شبکه\u200Cهای اجتماعی حضور فعالی دارید؟"],
        "Customer experience": ["5.آیا مشتریان احساس خوبی از ارتباط با شما و خرید محصولتان دارند؟",
                                "6.آیا شما از بازخورد مشتریان برای بهبود تجربه برند استفاده می\u200Cکنید؟"],
        "Brand loyalty": ["7.آیا شما برنامه\u200Cهایی برای افزایش وفاداری مشتریان به برند دارید؟",
                          "8.مشتریان چند بار خرید مجدد می\u200Cکنند؟",
                          "9.مشتریان حاضرند برای محصولات/خدمات شما نسبت به رقبا هزینه بیشتری پرداخت کنند؟"],
        "Brand evaluation": ["10. آیا شما ارزیابی دوره\u200Cای از عملکرد برند خود انجام می\u200Cدهید؟",
                             "11. آیا شما از داده\u200Cهای مشتریان برای سنجش عملکرد برند استفاده می\u200Cکنید؟"],
        "Brand strategy": ["12. آیا استراتژی برند شما مشخص و مستند است؟",
                           "13. آیا شما بر روی برند خود به\u200Cطور مستمر سرمایه\u200Cگذاری می\u200Cکنید؟"],
        "Innovation in branding": ["14. آیا شما در حال حاضر در حال نوآوری در برند خود هستید؟"],
    }
    medium_questions = {
        "هویت برند": [
...
class FinancialAPIVIew(APIView):
    ...
    def put(self, request):
        ...
        answer = request.data['answer']
        size = request.data['size']
        finall = "sales_change_percentage"
        if not user_id:
            return Response(data={"error": "userid is required"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request):
        nationalID = request.query_params.get('nationalID')
        if not nationalID:
            return Response(data={"nationalID is required"}, status=status.HTTP_400_BAD_REQUEST)

        user_id = request.query_params.get('userid')
        if not user_id:
            return Response(data={"userid is required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        answers = Financial.objects.filter(company=company).last()

        prompt = f'''تصور کن یک کارشناس مالی هستی و قصد داری بر اساس مدل دوپونت و با توجه به اطلاعات صورت سود و زیان و ترازنامه،
                یک تحلیل مالی برای شرکت ارائه بدی. اطلاعات مالی به شرح زیر است:
                .1میزان فروش خالص (درآمد کل)
                {answers.net_sales}

                .2سود خالص دوره (پس از کسر همه هزینهها، مالیات و بهره)
                {answers.net_profit_period}
                .3مجموع داراییها در پایان دوره:
                {answers.total_assets_end_period}
                .4مقدار میانگین داراییها طی سال (در صورت دسترسی:)
                {answers.average_assets_year}
                .5حقوق صاحبان سهام در پایان دوره:
                {answers.equity_end_period}
                .6میزان هزینههای مالی (بهره وامها و بدهیها:)
                {answers.financial_expenses}
                .7درصد تغییر فروش نسبت به سال قبل (اگر اطلاع داری:)
                {answers.sales_change_percentage}
                با توجه به این اطلاعات، تحلیل دوپونت را ارائه بده.
                در ابتدای گزارش تحلیلی خود، در  4پارگراف و حداقل  300کلمه در باره ی ضرورت تحلیل صورت های مالی، توضیح
                مدل دوپونت و استفاده از این مدل برای تحلیل اطلاعات وارد شده توضیح بده.هر یک از شاخص های حاشیه سود خالص، گردش دارایی ها و اهرم مالی را به صورت منحصر به فرد در  2پاراگراف و 120
                کلمه تحلیل کن
                همچنین برای تاثیرگذاری بهتر تحلیل در تصمیم گیری ها، راهکارهایی برای بهبود نسبت هایی که محاسبه کردی ارائه بده .
                فرمت ارائه راهکارها به شکل زیر باشد:
                برای هر شاخص ،  3تا  5پیشنهاد منحصر به فرد برای بهبود عملکرد آن شاخص به گونه ای که برای صاحب کسب و کار
                که سطح آشنایی اولیه در حوزه مدیریت مالی دارد، به صورت ساده ارائه بده
                یک بخش از گزارش که با بعد از توضیحات مقدماتی و قبل از تحلیل ها باشد را به محاسبات دوپونت اختصاص بده

        '''

        rule = '''
        تصور کن یک کارشناس مالی هستی و قصد داری بر اساس مدل دوپونت و با توجه به اطلاعات صورت سود و زیان و ترازنامه،
یک تحلیل مالی برای شرکت ارائه بدی
        '''
